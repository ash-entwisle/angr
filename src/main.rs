...
fn main() {
    ...
    // TODO: fix piping input
    let stdin = io::stdin();
    let mut pipe: String = "".to_string();
    ...
    let file: &String   = matches.get_one::<String>("file").unwrap_or(&blank);
    ...
    let text = std::fs::read_to_string(file).unwrap_or(pipe);
